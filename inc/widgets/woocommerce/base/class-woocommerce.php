...
namespace Boostify_Elementor\Woocommerce\Base;

use Boostify_Elementor\Base_Widget;
use Boostify_Elementor\Group_Control_Product;
...
/**
 * Abstract settings for woocemmerce
 *
 * @since 1.0.0
 * @package Boostify Addon
 */
abstract class Woocommerce extends Base_Widget {
	...
	protected function query_control() {
		...
		$this->add_group_control(
			Group_Control_Product::get_type(),
			array(
				'name' => 'product_group',
			)
		);
	 ...
	 * Pagination Style
	 *
	 * @param array $settings | widget setting array.
	 * @access protected
	 * @since 1.0.0
	 */
	protected function query_args( $settings ) {

		$include        = $settings['product_group_include'];
		$include_author = $settings['product_group_include_authors'];
		$in_term_key    = 'product_group_include_term_product';
		$include_term   = $settings[ $in_term_key ];
		$exclude        = $settings['product_group_exclude'];
		$exclude_author = $settings['product_group_exclude_authors'];
		$ex_term_key    = 'product_group_exclude_term_product';
		$exclude_term   = $settings[ $ex_term_key ];
		$ex_post_key    = 'product_group_exclude_posts_product';
